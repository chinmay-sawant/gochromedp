...
// ConvertURLToHTML converts a URL to HTML content
func ConvertURLToHTML(url string, options *ConvertOptions) (string, error) {
	if options == nil {
		options = DefaultConvertOptions()
	}
...
// ConvertHTMLToHTML converts HTML content to HTML (for consistency with other functions)
func ConvertHTMLToHTML(htmlContent string, options *ConvertOptions) (string, error) {
	if options == nil {
		options = DefaultConvertOptions()
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed to render HTML content: %v", err)
	}
